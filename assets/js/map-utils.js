(() => {
  ...
  function matchesAge(facility, selectedAge) {
    if (!selectedAge) {
      return true;
    }
    ...
    const ages = facility.ages || [];
    return ages.includes(targetAge);
  }

  function getWeekdaySlotLabels(facility, selectedWeekday) {
    if (!selectedWeekday) {
      return [];
    }
    const slots = facility.slotLabelsByWeekday || {};
    const labels = slots[selectedWeekday] || [];
    return labels.filter(Boolean);
  }

  function isWeekdayAvailable(facility, selectedWeekday) {
    if (!selectedWeekday) {
      return false;
    }
    const availability = facility.weekdayAvailability || {};
    return Boolean(availability[selectedWeekday]);
  }
  ...
  function getAvailabilityStatus(facility, selectedAge, selectedWeekday) {
    ...
    const ageMatch = matchesAge(facility, selectedAge);
    const dayMatch = isWeekdayAvailable(facility, selectedWeekday);
